...
const AwardListPage = async ({
	...
	const query: Prisma.AwardWhereInput = {};
			...
			if (value !== undefined) {
				switch (key) {
					...
					case "year":
						...
						break;
					case "category":
						query.category = { contains: value, mode: "insensitive" };
						break;
					default:
						break;
				}
	...
	const [data, count] = await prisma.$transaction([
		...
		prisma.award.count({ where: query }),
	]);
	...
	const renderRow = (item: AwardList) => (
		<tr
			key={item.id}
		...
		>
			...
			<td className="hidden lg:table-cell text-xs">{item.organization}</td>
			<td className="hidden md:table-cell">
				<span
			...
			<td>
				<div className="flex items-center gap-2">
					<FormModal table="award" type="update" data={item} />
					<FormModal table="award" type="delete" id={item.id} />
				</div>
			</td>
		</tr>
	);
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
					...
					<div className="flex items-center gap-4 self-end">
						...
						<FormModal table="award" type="create" />
					</div>
				</div>
			</div>
